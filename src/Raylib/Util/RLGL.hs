...
rlMultMatrixf :: (PALike CFloat matrix) => matrix -> IO ()
rlMultMatrixf matf = withALike matf c'rlMultMatrixf
...
rlGetShaderLocsDefault :: (PALike CInt locs) => IO locs
rlGetShaderLocsDefault = popALike 32 =<< c'rlGetShaderLocsDefault
...
rlDrawVertexArrayElements :: (PALike CUShort buffer) => Int -> buffer -> IO ()
rlDrawVertexArrayElements offset buffer =
  withALikeLen
    buffer
    (\l p -> c'rlDrawVertexArrayElements (fromIntegral offset) (fromIntegral l) (castPtr (p :: Ptr CUShort)))
...
rlDrawVertexArrayElementsInstanced :: (PALike CUShort buffer) => Int -> buffer -> Int -> IO ()
rlDrawVertexArrayElementsInstanced offset buffer instances =
  withALikeLen
    buffer
    ( \l p ->
        c'rlDrawVertexArrayElementsInstanced (fromIntegral offset) (fromIntegral l) (castPtr (p :: Ptr CUShort)) (fromIntegral instances)
    )

-- | Load texture in GPU
rlLoadTexture :: (PALike CUShort buffer) => buffer -> Int -> Int -> RLPixelFormat -> Int -> IO Integer
rlLoadTexture tData width height format mipmapCount =
  fromIntegral
    <$> withALike
      tData
      (\p -> c'rlLoadTexture (castPtr (p :: Ptr CUShort)) (fromIntegral width) (fromIntegral height) (fromIntegral $ fromEnum format) (fromIntegral mipmapCount))
...
rlLoadTextureCubemap :: (PALike CUShort buffer) => buffer -> RLPixelFormat -> IO Integer
rlLoadTextureCubemap tData format =
  fromIntegral
    <$> withALikeLen tData (\l p -> c'rlLoadTextureCubemap (castPtr (p :: Ptr CUShort)) (fromIntegral $ l * sizeOf (0 :: CUShort)) (fromIntegral $ fromEnum format))
...
rlReadTexturePixels :: (PALike CUChar contents) => Integer -> Int -> Int -> RLPixelFormat -> IO contents
rlReadTexturePixels tId width height format = do
  ...
  popALike size (castPtr ptr :: Ptr CUChar)

-- | Read screen pixel data (color buffer)
rlReadScreenPixels :: (PALike CUChar contents) => Int -> Int -> IO contents
rlReadScreenPixels width height =
  c'rlReadScreenPixels (fromIntegral width) (fromIntegral height) >>= popALike (width * height * 4)
...
rlSetUniformMatrices :: (PALike Matrix matrices) => Int -> matrices -> IO ()
rlSetUniformMatrices locIndex mats = withALikeLen mats (\c m -> c'rlSetUniformMatrices (fromIntegral locIndex) m (fromIntegral c))
...
rlSetShader :: (PALike CInt locs) => Integer -> locs -> IO ()
rlSetShader shaderId locs = withALike locs (c'rlSetShader (fromIntegral shaderId))
